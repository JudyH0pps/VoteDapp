...
contract Voting {
  ...
  function votedCheck(uint8 roomNumber, uint8 xx, uint8 yy, uint8 zz) view public returns (uint8) {
	return voteRoomList[roomNumber].voted[msg.sender];
  }
  
  function addVoteRoom(bytes32 _roomName, bytes32[] memory _candidateList, bytes32 _voteDate, uint8 xx) public {
  ...
  function getRoomName(uint8 roomNumber, uint8 xx) view public returns(bytes32) {
	return voteRoomList[roomNumber].roomName;
  }

  function getCandidateList(uint8 roomNumber, uint8 xx) view public returns(bytes32[] memory) {
	return voteRoomList[roomNumber].candidateList;
  }

  function getVoteDate(uint8 roomNumber, uint8 xx) view public returns(bytes32) {
	return voteRoomList[roomNumber].voteDate;
  }

  function voteForCandidate(uint8 roomNumber, uint8 candiNum, uint8 xx) public {
	require(votedCheck(roomNumber,0,0) != 1);
